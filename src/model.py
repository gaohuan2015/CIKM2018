...
class Encoder(nn.Module):
    ...
    def forward(self, sentence, pos):
        ...
        word_embedding = self.embedding(sentence).view(-1, 1, self.sentence_length, self.word_embedding_size)
        pos1_embedding = self.pos_embedding(pos).view(-1, 1, 1, 2 * self.pos_embedding_size)

        input_embedding = torch.cat(word_embedding, pos1_embedding, -1)
