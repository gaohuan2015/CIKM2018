...
def build_data(train_hrt_bags, word2id, entity2id, mention_id_map, list_size=70):
    # 找出所有一个实体对不止一个关系的数据
    ...
    for key, value in train_hrt_bags.items():
        ...
        e1_tid = mention_id_map[e1] if mention_id_map[e1] in entity2id.keys() else "UNK"
        e2_tid = mention_id_map[e2] if mention_id_map[e2] in entity2id.keys() else "UNK"
...
def build_path(train_ht_relation, train_hrt_bags, train_head_set, train_tail_set, bag2id, mention_id_map,
               entity2id_map):
    # build path data
    train_path = dict()
    ...
    with codecs.open("../data/path/path_tmp.txt", "w") as f:
        ...
        for e1, e2, relation in bag2id:
            ...
            mid = mention_id_map[tmp] if mention_id_map[e1] in entity2id.keys() else "UNK"
...
def load_entity_embedding():
    entity2id = dict()
    lines = [s for s in codecs.open("../data/kg_emb/entity2id.txt", "r", encoding="UTF-8").readlines()]
...
def load_kg_relation_embedding():
    re2id = dict()
    lines = [s for s in codecs.open("../data/kg_emb/kg_re2id.txt", "r", encoding="UTF-8").readlines()]
...
def init():
    ...
    data, label, pos1, pos2, bag2id, entity = build_data(train_hrt_bags, id2, emb_entity2id, mention_id_map)

    path_id, mid_entity = build_path(train_ht_relation, train_hrt_bags, train_head_set, train_tail_set, bag2id,
                                     mention_id_map,
                                     emb_entity2id)

    ht_relations, ht_path = kg_path()
    train_kg_path = list()
    ...
    for key, value in train_hrt_bags.items():
        ...
        train_kg_mid_entity.append(int(emb_entity2id[mid]) if mid in entity2id.keys() else int(emb_entity2id["UNK"]))
        train_kg_relation.append(
            [int(emb_kg_re2id[ra]) if ra in emb_kg_re2id.keys() else int(emb_kg_re2id["UNK"]),
             int(emb_kg_re2id[rb]) if rb in emb_kg_re2id.keys() else int(emb_kg_re2id["UNK"])])
        train_kg_path.append((e1, e2, mid))

    np.save("../data/path/train_kg_mid_entity.npy", np.array(train_kg_mid_entity))
    np.save("../data/path/train_kg_path_relation.npy", np.array(train_kg_relation))
    ...
    path_id, mid_entity = build_path(test_ht_bags, test_hrt_bags, test_head_set, test_tail_set, bag2id, mention_id_map,
                                     emb_entity2id)
    ...
    for key, value in test_hrt_bags.items():
        ...
        test_kg_mid_entity.append(int(emb_entity2id[mid]) if mid in entity2id.keys() else int(emb_entity2id["UNK"]))
        test_kg_relation.append(
            [int(emb_kg_re2id[ra]) if ra in emb_kg_re2id.keys() else int(emb_kg_re2id["UNK"]),
             int(emb_kg_re2id[rb]) if rb in emb_kg_re2id.keys() else int(emb_kg_re2id["UNK"])])
        test_kg_path.append((e1, e2, mid))

    np.save("../data/path/test_kg_mid_entity.npy", np.array(test_kg_mid_entity))
    np.save("../data/path/test_kg_path_relation.npy", np.array(test_kg_relation))
...
def kg_path():
    ...
    for line in lines:
        ...
        if (e1, e2) in ht_relations.keys():
            ht_relations[(e1, e2)].append(relation)
        else:
